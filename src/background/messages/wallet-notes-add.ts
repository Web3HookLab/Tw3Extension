...
export interface WalletNotesAddRequest {
...
export interface WalletNotesAddResponse {
  success: boolean
  data?: any
  error?: string
}
...
const handler: PlasmoMessaging.MessageHandler<WalletNotesAddRequest, WalletNotesAddResponse> = async (req, res) => {
  console.log('🔄 后台：处理添加钱包备注请求:', req.body)
  ...
  try {
    ...
    if (!token) {
      console.error('❌ 添加钱包备注失败: 未找到认证token')
      res.send({
        success: false,
        error: '用户未登录，请先登录后重试'
      })
    ...
    // 调用API添加钱包备注
    const apiUrl = `${API_CONFIG.BASE}${API_CONFIG.ENDPOINTS.WALLET_NOTES_ADD}`
    console.log('🌐 发送添加请求到:', apiUrl)
    ...
    if (data.code === 200) {
      console.log('✅ 钱包备注添加成功:', walletAddress, '备注内容:', note)
      ...
      console.error('❌ 钱包备注添加失败:', data.msg || '未知错误')
      res.send({
        success: false,
        error: data.msg || '添加钱包备注失败'
      })
    }
