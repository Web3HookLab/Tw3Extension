...
export interface TwitterNotesUpdateRequest {
  restId: string
  note: string
  tags: string[]
}

export interface TwitterNotesUpdateResponse {
  success: boolean
  data?: any
  error?: string
}

const handler: PlasmoMessaging.MessageHandler<TwitterNotesUpdateRequest, TwitterNotesUpdateResponse> = async (req, res) => {
  console.log('🔄 后台：处理更新Twitter备注请求:', req.body)
  ...
  try {
    ...
    // 验证参数
    if (!restId || !note) {
      console.error('❌ 更新备注参数验证失败:', { restId: !!restId, note: !!note })
      res.send({
        success: false,
        error: '参数不完整'
      })
    ...
    if (!token) {
      console.error('❌ 更新备注失败: 未找到认证token')
      res.send({
        success: false,
        error: '用户未登录，请先登录后重试'
      })
    ...
    // 调用API更新备注
    const apiUrl = `${API_CONFIG.BASE}${API_CONFIG.ENDPOINTS.TWITTER_NOTES_UPDATE}`
    ...
    if (data.code === 200) {
      console.log('✅ Twitter备注更新成功:', restId, '新备注内容:', note, '新标签:', tags)
      ...
      console.error('❌ Twitter备注更新失败 - API返回错误:', data)
      res.send({
        success: false,
        error: data.msg || '更新备注失败'
      })
    }

  } catch (error) {
    console.error('❌ 更新Twitter备注时出错:', error)
